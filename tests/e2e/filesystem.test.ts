...
function parseRequest(
  message: Buffer,
): { requestId: number; body: Buffer; dlen: number } {
...
function createFileSystemServer(): Promise<
...
describe("E2E: FileSystem lifecycle", () => {
  it("mkdir -> readdir -> mv -> rm -> rmdir", async () => {
    ...
    try {
      ...
      assert.ok(
        names1.includes("newdir"),
        `should contain newdir, got: ${names1.join(", ")}`,
      );
      ...
      assert.ok(
        names2.includes("renameddir"),
        `should contain renameddir, got: ${names2.join(", ")}`,
      );
