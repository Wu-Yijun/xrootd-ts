...
### 2. mkdir on existing path with different mode throws 3018
...
### 3. mkdir on existing path with same mode succeeds (idempotent)
...
### 6. rmdir on non-existent path succeeds (idempotent)
...
### 8. rm on non-existent path throws XRootDError code 3011
...
### 2. mv on non-existent source throws XRootDError code 3011
...
### 3. readdir on non-existent path throws error
...
### 4. readdir entries have correct fields (name, size, flags, mtime)
