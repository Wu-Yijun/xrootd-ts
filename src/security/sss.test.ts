...
describe('SSSAuth', () => {
  ...
  it('returns encrypted credentials', async () => {
    ...
    assert.ok(creds.length % 8 === 0) // Blowfish block alignment
  })
  ...
  it('different passwords produce different credentials', async () => {
    ...
    const creds2 = await auth2.getCredentials({ ...defaultParams, password: 'different' })
  ...
  it('handles empty password', async () => {
    const auth = new SSSAuth(testKey)
    const creds = await auth.getCredentials({ ...defaultParams, password: '' })
